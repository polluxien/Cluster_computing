...
int main(int argc, char **argv)
{
    ...
    if (workerRank == 0)
    {
        Timestamp singleComputeStartTime = timestampNow();
        ...
        timestampAddSince(&singleComputeTime, singleComputeStartTime);
    }

    // Broadcast points.
    Timestamp commStartTime = timestampNow();
    MPI_Bcast(&pointCount, 1, MPI_INT, 0, MPI_COMM_WORLD);
    timestampAddSince(&commTime, commStartTime);
    ...
    for (int iteration = 0; iteration < simIterCount; iteration++)
    {
        ...
        commStartTime = timestampNow();
        MPI_Bcast(points, pointCount * sizeof(Point), MPI_CHAR, 0, MPI_COMM_WORLD);
        timestampAddSince(&commTime, commStartTime);

        // Update own slice of points.
        Timestamp multiComputeStartTime = timestampNow();
        pointsUpdateAccelerationAndVelocity(workerRange, points, pointCount, simStep);
        pointsMove(points + workerRange.low, workerRange.high - workerRange.low, simStep);
        timestampAddSince(&multiComputeTime, multiComputeStartTime);
        ...
        if (workerRank == 0 && imageGenerationOn)
        {
            Timestamp imageStartTime = timestampNow();
            char filepath[256] = {0};
            ...
            timestampAddSince(&imageTime, imageStartTime);
        }
    }
